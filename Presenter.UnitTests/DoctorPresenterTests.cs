...
namespace KooliProjekt.WinFormsApp.Tests
{
    public class DoctorPresenterTests
    {
        ...
        [Fact]
        public async Task Initialize_ShouldLoadDoctors()
        {
            // Arrange
            ...
            // Assert
            _mockView.VerifySet(x => x.Doctors = doctors, Times.Once);
        }
        ...
        public async Task DeleteDoctor_WhenUserConfirms_ShouldCallApiAndShowSuccess()
        {
            ...
            _mockApiClient.Setup(x => x.Delete(doctorId)).ReturnsAsync(successResult);
        ...
        public async Task SaveDoctor_WhenValid_ShouldCallApiAndShowSuccess()
        {
            ...
            var successResult = new Result();
            _mockApiClient.Setup(x => x.Save(It.IsAny<Doctor>())
                         .ReturnsAsync(successResult)
                         .Callback<Doctor>(d =>
                         {
                             Assert.Equal(doctorId, d.Id);
                             Assert.Equal(name, d.Name);
                             Assert.Equal(specialization, d.Specialization);
                         });
            ...
            // Assert
            _mockApiClient.Verify(x => x.Save(It.IsAny<Doctor>()), Times.Once);
            _mockView.Verify(x => x.ShowMessage(
